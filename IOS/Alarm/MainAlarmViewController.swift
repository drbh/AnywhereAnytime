...
class MainAlarmViewController: UITableViewController{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isEditing {
            performSegue(withIdentifier: Id.editSegueIdentifier, sender: SegueInfo(curCellIndex: indexPath.row, isEditMode: true, label: alarmModel.alarms[indexPath.row].label, tzStr:  alarmModel.alarms[indexPath.row].tzStr, mediaLabel: alarmModel.alarms[indexPath.row].mediaLabel, mediaID: alarmModel.alarms[indexPath.row].mediaID, repeatWeekdays: alarmModel.alarms[indexPath.row].repeatWeekdays, enabled: alarmModel.alarms[indexPath.row].enabled, snoozeEnabled: alarmModel.alarms[indexPath.row].snoozeEnabled))
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let largFontSize: CGFloat = 35.0
        var rightHandFontSize: CGFloat = 15.0
        if alarm.formattedTime.count <= 5 {
            rightHandFontSize = largFontSize
        }
//        print(alarm.formattedTime, alarm.formattedTime.count)
        
        let amAttr: [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue) : UIFont.systemFont(ofSize: rightHandFontSize)]
        let str = NSMutableAttributedString(string: alarm.formattedTime, attributes: amAttr)
        

        let timeAttr: [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue) : UIFont.systemFont(ofSize: largFontSize)]
        str.addAttributes(timeAttr, range: NSMakeRange(0, str.length-2))
//        cell!.textLabel?.attributedText = str
        
        let XamAttr: [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue) : UIFont.systemFont(ofSize: rightHandFontSize)]
        ...
        let Xstr = NSMutableAttributedString(string: pad  + alarm.formattedTimeOri, attributes: XamAttr)
        
        let XtimeAttr: [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue:  NSAttributedStringKey.font.rawValue) : UIFont.systemFont(ofSize: largFontSize)]
        Xstr.addAttributes(XtimeAttr, range: NSMakeRange(0, Xstr.length-2))
        cell!.textLabel?.attributedText = str + Xstr
        
        
        let tmzs = "(" + TimeZone.current.abbreviation()! + " time)                         ("+alarm.tzStr+" time)   |   "
        cell!.detailTextLabel?.text = tmzs + alarm.label
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Id.addSegueIdentifier {
            ...
            addEditController.segueInfo = SegueInfo(curCellIndex: alarmModel.count, isEditMode: false, label: "Alarm", tzStr: "GMT", mediaLabel: "bell", mediaID: "", repeatWeekdays: [], enabled: false, snoozeEnabled: false)
        }
